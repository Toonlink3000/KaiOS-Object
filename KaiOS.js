...
CRunKaiOS.CND_ADWASCLOSED = 7; //done
CRunKaiOS.CND_ADWASCLICKED = 8; //done
CRunKaiOS.CND_IFERRORDISPLAYAD = 9; //done
...
CRunKaiOS.ACT_LOADDISPLAYAD = 8; //done
    ...
    condition: function(num, cnd) {
        switch (num) {
            ...
            case CRunKaiOS.CND_WHILECHARGING:
                let charging = navigator.battery.charging;
                return charging;
                break;

            case CRunKaiOS.CND_ISKAIOS:
                let version = cnd.getParamExpression(this.rh, 0)
                let isKaiV = false;
                if (isKaiOS) {
                    switch (version) {
                        // Any version
    ...
    action: function(num, act) {
        switch (num) {
            ...
            case CRunKaiOS.ACT_RUMBLEDEVICE:
                let duration = act.getParamExpression(this.rh, 0);
            ...
            case CRunKaiOS.ACT_LOADDISPLAYAD:
                ...
                let pubid = act.getParamExpression(this.rh, 0);
                let appnm = act.getParamExpression(this.rh, 1);
                let timeout = act.getParamExpression(this.rh, 2);
                let testingnum = act.getParamExpression(this.rh, 3);
                let testing = 0;
        ...
        try {
            switch (num) {
                ...
                case CRunKaiOS.EXP_BATTERYLEVEL:
                    let level = navigator.battery.level;
                    return level * 100;
                    break;
                ...
                case CRunKaiOS.EXP_CHARGETIME:
                    let ctime = navigator.battery.chargingTime;
                    return ctime;
                    break;

                case CRunKaiOS.EXP_DISCHARGETIME:
                    let dtime = navigator.battery.dischargingTime;
                    return dtime;
                    break;

                case CRunKaiOS.EXP_BATTERYTEMPERATURE:
                    let temperature = navigator.battery.temperature;
                    return temperature;
                    break;
